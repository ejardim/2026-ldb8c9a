...
fit1 <- sca(stock,tun.sel[c(1,4)],fmodel=fmod,qmodel=qmod,srmodel=srmod)
...
res <- residuals(fit1, stock, tun.sel[c(1,4)])
...
#res$LPUE.LCGOTBDEF_1[is.na(res$LPUE.LCGOTBDEF_1)] <- 0 #hack
#res$LPUE.LCGOTBDEF_2[is.na(res$LPUE.LCGOTBDEF_2)] <- 0 #hack
...
res <- residuals(fit1, stock, tun.sel[c(1,4)],type = "pearson")
plot(res)
SavePlot('Residuals4')

wireframe(fit1@harvest)
a <- xyplot(data~age,groups=year,stk1@harvest,type='b',ylim=c(0,1.25),ylab='F',main='Fishing mortality')
a
SavePlot('F')
...
a1 <- xyplot(data~age,groups=year,data=fitted$qmodel[1],type='b',ylab='Catchabilty',main='Q SPGFS',ylim=c(0.000,0.0015))
a1
#a2 <- xyplot(data~age,groups=year,data=fitted$qmodel[2],type='b',ylab='Catchabilty',main='Q LPUE.LCGOTBDEF_1',ylim=c(0,7))
#a2
#a3 <- xyplot(data~age,groups=year,data=fitted$qmodel[3],type='b',ylab='Catchabilty',main='Q LPUE.LCGOTBDEF_2',ylim=c(0,0.06))
#a3
a2 <- xyplot(data~age,groups=year,data=fitted$qmodel[2],type='b',ylab='Catchabilty',main='Q PTCRUST',ylim=c(0.000,0.0010))
a2
...
nyears <- length(years)
#GM <- exp(mean(log(window(stock.n(stk1)["0",], start=1990, end=2022))))
#GM
#[1] 44393.69

GM <- exp(mean(log(window(stock.n(stk1)["0",], start=2023, end=2025)))) #this WG 2026 option
GM
#[1] 22747.69
...
retro <- lapply(retro, function(x){
  ...
  tun <- window(tun.sel[c(1,4)], end=yr)
...
with(kobe[1,],plot(B/Btrig,F/Fmsy,xlim=c(0.5,7),ylim=c(0,4.0),main='ldb8c9a'))
rect(-1,-1,7,6,col='yellow',border=NA)
rect(-1,1,1,6,col='red',border=NA)
rect(1,-1,7,1,col='green',border=NA)
