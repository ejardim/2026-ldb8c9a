...
#fit1 <- sca(stock,idx],fmodel=fmod,qmodel=qmod,srmodel=srmod)
...
SavePlot('F_3D')
a <- xyplot(data~age,groups=year,stk1@harvest,type='b',ylim=c(0,0.6),ylab='F',main='Fishing mortality') #sin leyenda
a
SavePlot('F')
...
a2 <- xyplot(data~age,groups=year,data=fitted$qmodel[2],type='b',ylab='Catchabilty',main='Q PTCRUST',ylim=c(0.000,0.0005))
a2
...
#F (Average F(last 3 years)), R geometric mean (1990:2019))
...
#retro
#fit1 <- sca(stk, idx], fmodel=fmod, qmodel=qmod, srmodel=srmod)
#stk1 <- stk + fit1
back <- 5
...
with(kobe[1,],plot(B/Btrig,F/Fmsy,xlim=c(0.5,15),ylim=c(0,4.0),main='ldb8c9a'))
rect(-1,-1,15,6,col='yellow',border=NA)
rect(-1,1,1,6,col='red',border=NA)
rect(1,-1,15,1,col='green',border=NA)
